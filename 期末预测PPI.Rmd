...
In comparison, LASSO has a higher R square and the smallest mse, so LASSO was chosen to anticipate PPI infaltion rate.

# Import the real test set
```{r warning=FALSE }
...
Convert the true value into a time series
```{r}
library(zoo)
library(xts)
years_1 <- floor((real.X$month - 1) / 12) + 2000
months_1<- (real.X$month - 1) %% 12 + 1
dates_1 <- as.Date(paste0(years_1, "-", months_1, "-01"))
real.X <- xts(real.X[, -1], order.by = dates_1)
...
# apply normalization to entire data frame
real.X_standard <- as.data.frame(apply(real.X,2,normalize))
```

Select the variables selected in the preprocessing in the real test set
```{r}
real_X<- subset(real.X_standard, select = c(stable_vars))
```
...
cv.fit$lambda.1se
coef(cv.fit,s=cv.fit$lambda.1se)
lasso.pred=predict(cv.fit,newx=real_X, s=cv.fit$lambda.1se)  
```
# Here are the 30 values we predicted
```{r warning=FALSE }
lasso.pred
```

Prediction effect based on true value
```{r}

merged_ppi<-rbind(ppi,real.ppi)
# Calculated year-on-year growth rate of ppi（inflation rate）
ppi_yoy_1 <- ((merged_ppi[13:198, 2] / merged_ppi[1:186, 2]) - 1)*100

ppi_yoy_df_1 <- data.frame(month = merged_ppi$month[13:198], PPI_YoY = ppi_yoy_1)

years_2 <- floor((ppi_yoy_df_1$month - 1) / 12) + 2000
months_2<- (ppi_yoy_df_1$month - 1) %% 12 + 1
dates_2 <- as.Date(paste0(years_2, "-", months_2, "-01"))
ppi_yoy_df_1_real<- xts(ppi_yoy_df_1[, -1], order.by = dates_2)

mse_lasso_1<-mean((lasso.pred-ppi_yoy_df_1_real[157:186])^2)
mse_lasso_1
r_squared_Lasso_1<- cor(lasso.pred, ppi_yoy_df_1_real[157:186])^2
r_squared_Lasso_1
```

Acquire out-of-sample R-squared
```{r}
R_squared_OOS<-1-var(ppi_yoy_df_1_real[157:186]-lasso.pred)/var(ppi_yoy_df_1_real[157:186])
R_squared_OOS
```
